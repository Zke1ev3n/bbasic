...
int Assembler::GetSymbol(int IdentP)
{
    long v_i=1;
    float v_f=1;
    SymError=0;
...
void Assembler::SetDWord(int ip,vdword value)
{
...
int Assembler::MakeOneSymbol(int ip,int IdentP)
{
    ErrorMsg[0]=0;
    SymPreType=DWord;
    GetSymbol(IdentP);
...
int Assembler::MakeTwoSymbol(int ip,int IdentP)
{
    ErrorMsg[0]=0;
    SymPreType=DWord;
...
//int Assembler::LoadAsm(string &filename) {}

int Assembler::LoadAsm(stringstream &stream) {
    int  ip=0;
    int  i=0,j=0,c=0,k=0;
    ...
    for (LoadOK=0;(stream.eof()==0&&!LoadOK);LinePoint++) {
        string line = "";
        for (c=stream.get();c&&c!='\n'&&c!=-1&&!stream.eof();c=stream.get()){
            //line.append(c);
            line += c;
        }
        IdentReader.FindIdent(line.c_str(),KeyWords); // 对这行代码词法分析
        if (IdentReader.GetCount())		// 本行有代码
        {
            string AsmCmd,CmdDat;
            ...
            AsmCmd = IdentReader[Ident]->sValue;
            CmdDat = IdentReader[Ident+1]->sValue;
            for (i=0;i<AsmCmd.size();i++) if (AsmCmd[i]>='a'&&AsmCmd[i]<='z') AsmCmd[i]=AsmCmd[i]-'a'+'A';
            for (i=0;i<CmdDat.size();i++) if (CmdDat[i]>='a'&&CmdDat[i]<='z') CmdDat[i]=CmdDat[i]-'a'+'A';
            if (CmdDat==":") {			// 地址标识
                int FindMatch;
            ...
            if (AsmCmd=="JPC") {
                ...
                if (CmdDat=="BE") Code[ip]+=3; else {
                    LoadOK=-3;
                    sprintf(AsmError,"Error unknow condition jump command %s",CmdDat.c_str());
                    break;
                }
                ip+=6;
            }	else
            ...
            if (AsmCmd=="CAL") {
                ...
                sprintf(AsmError,"Error (%d) unknow instruction %s",LinePoint,AsmCmd.c_str());
                LoadOK=-4;
                break;
            }
        }
...
int Assembler::Output(string filename) {
